...
@section('pegawai')
    <div>
        <!-- Search -->
        <div class="p-4">
            <form method="GET" action="{{ route('pegawai.miss.menu') }}">
        ...
        <div class="flex justify-between px-4 mb-4 space-x-2">
            <a href="{{ route('pegawai.miss.menu', ['kategori' => 'Miss Smoothies']) }}"
                class="flex-1 px-3 py-2 border rounded-md text-sm
                      {{ request('kategori') === 'Miss Hotang' ? 'bg-white text-gray-700' : 'bg-purple-500 text-white' }}">
                Miss Smoothies
            </a>
            <a href="{{ route('pegawai.miss.menu', ['kategori' => 'Miss Hotang']) }}"
                class="flex-1 px-3 py-2 border rounded-md text-sm
                      {{ request('kategori') === 'Miss Hotang' ? 'bg-purple-500 text-white' : 'bg-white text-gray-700' }}">
                Miss Hotang
            </a>
        </div>
        ...
        <div class="p-4 mb-4">
            ...
            <form method="GET" action="{{ route('pegawai.miss.menu') }}">
                ...
                @endif
                <select name="kategori_nama" id="filter_kategori" class="w-full border rounded-lg p-2" onchange="this.form.submit()">
                    <option value="">-- Semua Kategori --</option>
                    @foreach ($kategoriList as $kategori)
                        <option value="{{ $kategori->nama }}" {{ request('kategori_nama') == $kategori->nama ? 'selected' : '' }}>
                            {{ $kategori->nama }}
                        </option>
                    @endforeach
                </select>
                ...
                <div
                    ...
                    <div class="px-2 mb-5 flex flex-col justify-between flex-grow items-center">
                        ...
                        <div class="flex flex-col justify-between flex-grow items-center">
                            <form action="{{ route('pegawai.miss.keranjang.add', $menu->id) }}" method="POST"
    ...
    <script>
        const btnSmoothies = document.getElementById('btn-smoothies');
        const btnHotang = document.getElementById('btn-hotang');

        function setActive(activeBtn, inactiveBtn) {
            activeBtn.classList.add('bg-purple-500', 'text-white');
            activeBtn.classList.remove('bg-white', 'text-gray-700');

            inactiveBtn.classList.add('bg-white', 'text-gray-700');
            inactiveBtn.classList.remove('bg-purple-500', 'text-white');
        }

        btnSmoothies.addEventListener('click', () => {
            setActive(btnSmoothies, btnHotang);
        });

        btnHotang.addEventListener('click', () => {
            setActive(btnHotang, btnSmoothies);
        });
    </script>
@endsection
