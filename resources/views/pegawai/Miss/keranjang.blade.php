...
@section('pegawai')
    <div class="container mx-auto">
        <div class="mt-1 justify-between flex mb-3">
            <h1 class="font-bold">Keranjang Pesanan</h1>
        </div>
        ...
        <div class="fixed bottom-0 left-0 right-0 bg-white p-4">
            <form action="{{ route('pegawai.miss.keranjang.checkout') }}" method="POST">
                @csrf
                <div class="mb-4">
    ...
    <script>
        function updateQuantity(keranjangId, action) {
            fetch(`/pegawai/miss/keranjang/update-quantity/${keranjangId}`, {
                    method: 'POST',
                    headers: {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        alert('Gagal update keranjang: ' + (data.message || 'Terjadi kesalahan.'));
                    }
                })
                .catch(error => {
